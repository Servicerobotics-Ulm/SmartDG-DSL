...
ComponentAImpl::ComponentAImpl(const std::string &componentName, int & argc, char ** argv)
:	SmartACE::SmartComponent(componentName, argc, argv)
{  }

ComponentAImpl::ComponentAImpl(const std::string &componentName, int & argc, char ** argv, const ACE_Sched_Params &sched_params)
...
int ComponentAImpl::startComponentInfrastructure(void)
{
...
void ComponentAImpl::stopComponentInfrastructure(const std::chrono::steady_clock::duration &timeoutTime)
{
	this->signalSmartTasksToStop(timeoutTime);
}
void ComponentAImpl::cleanUpComponentResources()
{
